	...
	if (isset($_POST['page_submit'])) {
		try {
			...
			if (!empty($title) && !empty($content)) {
				...
				$update_pages = $conn->prepare($query);
				$update_pages->execute(array(':title' => $title, ':content' => $content));
				$message = "<p class=\"notify\">New page has been successfully created!</p>";
			} else {
				$message = "<p class=\"notify\">Failed to add new page</p>";
			}
	...
	<div id="contentWrap">
		<div>
			...
			<form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
				...
				<input type="submit" name="page_submit" class="osbutton" value="Publish" />&nbsp;
				<?php
					if (isset($message)) {
						echo $message;
					}
